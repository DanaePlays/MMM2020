...
local function finalPlacement()
    ...
    local playedSound = tes3.playItemPickupSound{ item = this.active.object, pickup = false }
    mwse.log("Played sound? %s",playedSound)
    endPlacement()
end
...
local function setVerticalMode(n)
    ...
    elseif (n == 4) then
        ...
        mwse.log("This shouldn't have happened")
        this.height = 0
    end
...
local function isShiftDown()
    local inputCtr = tes3.worldController.inputController
    return (
        inputCtr:isKeyDown(tes3.scanCode.lShift)
        or inputCtr:isKeyDown(tes3.scanCode.rShift)
    )
end
...
local function activatePlacement()
    ...
    local target = ray and ray.reference
    ...
    elseif (target) then
        ...
        --You can move statics and activators with shift held down
        if not isShiftDown() then
            if (not placeableTypes[target.object.objectType]) then
                return
            end
        else
            if (not placeableTypes[target.object.objectType]) 
                and (not shiftPlaceableTypes[target.object.objectType]) 
            then
                return
            end
        end
        ...
        if (itemdata and itemdata.owner) then
            ...
            if (owner.objectType == tes3.objectType.faction and owner.playerJoined and owner.playerRank >= itemdata.requirement) then
                ...
                mwse.log()
            elseif not config.allowOwned then
...
local function modeKeyDown(e)
    if (e.keyCode == config.keybind) then
        if this.active and tes3.worldController.inputController:isKeyDown(tes3.scanCode.lShift) then
            this.active.position = this.itemInitialPos
            this.active.orientation = this.itemInitialOri
            endPlacement()
            return
